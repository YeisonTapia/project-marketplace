<template>
  <q-page class="page">

    <div class="bg-fondo q-px-sm q-py-xl">
      <div class="q-container">
        <div class="row">
          <div class="col-12 q-my-xl">
            <img class="w-100" src="/statics/img/nosotros.jpg" alt="nosotros">
          </div>
          <div class="col-xs-12 col-sm-12 col-md-3 text-center q-py-md">
            <h1 class="title text-secondary font-family-secondary">Nosotros</h1>
            <img src="/statics/img/arrow-right-pink.png" alt="nosotros">
          </div>
          <div class="col-xs-12 col-sm-12 col-md-9 text-justify">
            <p class="text-weight-medium">Somos dos jóvenes soñadores que un día común y corriente mientras jugábamos un partido de futbol en FIFA 2015 en PlayStation queríamos pedir un domicilio de comida, pero no teníamos un número de contacto ni sabíamos de un buen lugar para comprarla. Al vernos en esta situación nos preguntamos: ¿Qué tal si hubiera una página web dedicada a la búsqueda y localización de todos los negocios de Riohacha, sin discriminación alguna? Fue así como nació la idea de dondeestaesavaina.com. Una  página web dedicada a usuarios que buscan información y localización de las empresas formales e informales y negocios de barrio de la ciudad de Riohacha, para adquirir productos y/o servicios de las mismas, quienes se ven beneficiadas con un reconocimiento y publicidad online, lo cual ayudara en el aumento de sus ventas.</p>
          </div>

        </div>
      </div>
    </div>

    <div class="q-container q-px-sm">
      <div class="row">
        <div class="col-12">

          <h3 class="title-label-pink text-center"><div> Valores de la Empresa</div></h3>

          <q-list class="border-0 q-py-xl">
            <q-item>
              <q-item-side class="q-pr-sm">
                <q-item-tile avatar>
                  <img src="/statics/img/arrow-right-pink.png">
                </q-item-tile>
              </q-item-side>
              <q-item-main>
                <q-item-tile label>
                  <div class="q-title text-weight-bold">Obsesión por la satisfacción  del cliente:</div>
                  <div class="q-subheading">Nuestra consigna es la satisfacción 100% del cliente.</div>
                </q-item-tile>
              </q-item-main>
            </q-item>
            <q-item>
              <q-item-side class="q-pr-sm">
                <q-item-tile avatar>
                  <img src="/statics/img/arrow-right-pink.png">
                </q-item-tile>
              </q-item-side>
              <q-item-main>
                <q-item-tile label>
                  <div class="q-title text-weight-bold">Creatividad e innovación:</div>
                  <div class="q-subheading">Buscamos ser creativos e innovar en nuestro día a día.</div>
                </q-item-tile>
              </q-item-main>
            </q-item>
            <q-item>
              <q-item-side class="q-pr-sm">
                <q-item-tile avatar>
                  <img src="/statics/img/arrow-right-pink.png">
                </q-item-tile>
              </q-item-side>
              <q-item-main>
                <q-item-tile label>
                  <div class="q-title text-weight-bold">Actitud positiva:</div>
                  <div class="q-subheading">Siempre tenemos la disposición de darlo todo por nuestros clientes.</div>
                </q-item-tile>
              </q-item-main>
            </q-item>
            <q-item>
              <q-item-side class="q-pr-sm">
                <q-item-tile avatar>
                  <img src="/statics/img/arrow-right-pink.png">
                </q-item-tile>
              </q-item-side>
              <q-item-main>
                <q-item-tile label>
                  <div class="q-title text-weight-bold">Pro actividad:</div>
                  <div class="q-subheading">Acción y no reacción.</div>
                </q-item-tile>
              </q-item-main>
            </q-item>
            <q-item>
              <q-item-side class="q-pr-sm">
                <q-item-tile avatar>
                  <img src="/statics/img/arrow-right-pink.png">
                </q-item-tile>
              </q-item-side>
              <q-item-main>
                <q-item-tile label>
                  <div class="q-title text-weight-bold">Servicio de calidad:</div>
                  <div class="q-subheading">Siempre buscamos brindar un servicio de excelencia y calidad.</div>
                </q-item-tile>
              </q-item-main>
            </q-item>
            <q-item>
              <q-item-side class="q-pr-sm">
                <q-item-tile avatar>
                  <img src="/statics/img/arrow-right-pink.png">
                </q-item-tile>
              </q-item-side>
              <q-item-main>
                <q-item-tile label>
                  <div class="q-title text-weight-bold">Transparencia:</div>
                  <div class="q-subheading">La honestidad ante todo, siempre mostrando nuestra verdadera cara ante nuestros clientes.</div>
                </q-item-tile>
              </q-item-main>
            </q-item>
            <q-item>
              <q-item-side class="q-pr-sm">
                <q-item-tile avatar>
                  <img src="/statics/img/arrow-right-pink.png">
                </q-item-tile>
              </q-item-side>
              <q-item-main>
                <q-item-tile label>
                  <div class="q-title text-weight-bold">Amistad:</div>
                  <div class="q-subheading">Trabajamos con el cliente, no para el cliente, buscamos que tenga participación activa.</div>
                </q-item-tile>
              </q-item-main>
            </q-item>
          </q-list>
        </div>
      </div>
    </div>

    <div class="q-pa-xl w-100 shadow-2">
      <div class="q-container">
        <div class="row gutter-lg">
          <div class="col-xs-12 col-sm-12 col-md-6">
           <q-card flat class="rounded-md bg-primary text-white w-100 h-100">
            <h3 class="title-label text-center bg-warning text-secondary q-mt-lg q-ml-20">
              <div>Misión</div>
            </h3>
            <q-card-main >
              <div class="q-subheading q-px-lg q-pb-lg text-justify">
                Impulsar el reconocimiento online de las empresas formales e informales y negocios de barrios de la ciudad de Riohacha, buscando con esto la satisfacción tanto de las empresas, clientes y personas naturales, quienes tendrán la facilidad de encontrar los sitios de sus preferencias de una manera más rápida, eficiente y dinámica, mediante los servicios ofrecidos en la página web, logrando con esto un cambio en la manera tradicional que existe en Riohacha de localizar los negocios de los cuales el usuario quiere adquirir los productos y/o servicios.
              </div>
            </q-card-main>
          </q-card>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-6">
           <q-card flat class="rounded-md bg-primary text-white w-100 h-100">
            <h3 class="title-label text-center bg-warning text-secondary q-mt-lg q-ml-20">
              <div>Visión</div>
            </h3>
            <q-card-main >
              <div class="q-subheading q-px-lg q-pb-lg text-justify">
                Ser la página web número uno en lo que se refiere a la búsqueda y localización de las  empresas formales e informales y negocios de barrios de la ciudad de Riohacha.
              </div>
            </q-card-main>
          </q-card>
        </div>

      </div>
    </div>
  </div>
</q-page>
</template>
<script>
export default {
  name: 'PageNosotros',
  components: {},
  data() {
    return {
      lang: this.$q.i18n.lang,
    }
  }
}
</script>
<style lang="stylus">
.q-ml-20
  margin-left -20px
</style>