<template>
   <div>
      <q-card class="card-store q-ma-md" v-if="$q.platform.is.desktop">
      ...
      <q-card class="card-store q-ma-md" v-else>
         <router-link :to="{name: 'stores.show',params: {slug: store.slug}}">
            <q-img :ratio="4/3" :src="store.logo.path">
            </q-img>
         </router-link>

            <q-card-section class="q-pa-xs text-subtitle1 text-bold text-primary label-title text-uppercase items-center">
              <router-link :to="{name: 'stores.show',params: {slug: store.slug}}">  {{store.name}}</router-link>
            </q-card-section>
