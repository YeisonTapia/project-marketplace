<template>
  <div id="formAuthPage"
       class="form-content shadow-1 q-px-md q-pb-lg bg-white rounded-sm">
    <!--Tabs to toogle between login and register form-->
    <q-tabs inverted v-model="tabModel" align="justify" color="secondary" class="q-mt-lg">
      <!-- Tabs Title -->
      <q-tab slot="title" name="tab-login" :label="$tr('quser.layout.label.login')"
             @select="metaTitle = 'Iniciar Sesión'" class="font-family-secondary"/>
      <q-tab slot="title" name="tab-register" :label="$tr('quser.layout.label.createAccount')"
             @select="metaTitle = 'Crear Cuenta'" v-if="withRegister" class="font-family-secondary"/>
